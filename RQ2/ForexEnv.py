import pandas as pd
import numpy as np
import gymnasium as gym # FinRL and SB3 now use Gymnasium
from gymnasium import spaces
# from stock_indicators import indicators # Replaced with stockstats
from stockstats import StockDataFrame # Import StockDataFrame
from stable_baselines3 import DQN
from stable_baselines3.common.vec_env import DummyVecEnv
# from stable_baselines3.common.callbacks import BaseCallback # For custom logging or actions during training (optional)
...
LOG_LEVEL = 1 # 0 for no logging from env, 1 for info, 2 for debug

# Technical Indicators Periods (stockstats uses these by appending to indicator name e.g. rsi_14)
RSI_PERIOD = 14
MACD_FAST_PERIOD = 12 # Default for stockstats macd
MACD_SLOW_PERIOD = 26 # Default for stockstats macd
MACD_SIGNAL_PERIOD = 9 # Default for stockstats macd (for macds and macdh)
...
class ForexEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.current_step == 500:
            print(f"Step {self.current_step}: Action history: {self.action_history[-10:]}") # Show last 10 actions
        ...
        if self.equity <= 0:
            terminated = True
            if self.log_level > 0: print(f"Step {self.current_step}: Agent ruined. Equity: {self.equity:.2f}")

        if self.current_step >= self.total_steps -1 :
            truncated = True
            if self.log_level > 0: print(f"Step {self.current_step}: End of data reached.")
        ...
        if self.log_level > 1:
             print(f"Step: {self.current_step}, Action: {action}, Bid: {current_bid:.5f}, Ask: {current_ask:.5f}, "
    ...
    def _execute_trade(self, action, bid_price, ask_price):
        if self.log_level > 1: print(f"Executing trade action: {action} at step {self.current_step}.")
        ...
        if action == 0: # Go to CASH
            if self.position == 1: # Was LONG
                ...
                if self.log_level > 0: print(f"Step {self.current_step}: Closing LONG. Sold {self.shares:.2f} at {bid_price:.5f}. Commission: {commission:.2f}. Cash: {self.cash:.2f}")
                ...
                if self.log_level > 0: print(f"Step {self.current_step}: Closing SHORT. Covered {abs(self.shares):.2f} at {ask_price:.5f}. Commission: {commission:.2f}. Cash: {self.cash:.2f}")
            ...
            if self.position == -1: # Close SHORT first
                ...
                commission = cost_to_cover * self.transaction_cost_pct
                self.cash -= (cost_to_cover + commission)
                if self.log_level > 0: print(f"Step {self.current_step}: Closing SHORT before LONG. Covered {abs(self.shares):.2f} at {ask_price:.5f}. Commission: {commission:.2f}. Cash: {self.cash:.2f}")
            ...
            if self.position == 0 and self.cash > 0:
                if ask_price <= 0 or (ask_price * (1 + self.transaction_cost_pct)) <= 0 :
                    if self.log_level > 0: print(f"Step {self.current_step}: Cannot go LONG, invalid ask price or total cost. Ask: {ask_price}")
                    return
                ...
                if self.log_level > 0: print(f"Step {self.current_step}: Opened LONG. Bought {self.shares:.2f} at {ask_price:.5f}. Commission: {commission:.2f}. Cash left: {self.cash:.2f}")
                self.trade_history.append((self.current_step, "OPEN_LONG", ask_price, self.shares, commission))
            elif self.cash <= 0 and self.log_level > 0:
                 print(f"Step {self.current_step}: Cannot go LONG, insufficient cash: {self.cash:.2f}")
            ...
            if self.position == 1: # Close LONG first
                ...
                commission = sell_value * self.transaction_cost_pct
                self.cash += sell_value - commission
                if self.log_level > 0: print(f"Step {self.current_step}: Closing LONG before SHORT. Sold {self.shares:.2f} at {bid_price:.5f}. Commission: {commission:.2f}. Cash: {self.cash:.2f}")
            ...
            if self.position == 0 and self.cash > 0:
                if bid_price <= 0 :
                    if self.log_level > 0: print(f"Step {self.current_step}: Cannot go SHORT, invalid bid price: {bid_price}")
                    return
                ...
                if self.log_level > 0: print(f"Step {self.current_step}: Opened SHORT. Shorted {abs(self.shares):.2f} at {bid_price:.5f}. Notional: {notional_value_to_short:.2f}. Commission: {commission:.2f}. Cash: {self.cash:.2f}")
                self.trade_history.append((self.current_step, "OPEN_SHORT", bid_price, abs(self.shares), commission))
            elif self.cash <=0 and self.log_level > 0:
                 print(f"Step {self.current_step}: Cannot go SHORT, insufficient cash for notional value: {self.cash:.2f}")

    def render(self, mode='human'):
        if mode == 'human':
            print(f"Step: {self.current_step}, Equity: {self.equity:.2f}, Shares: {self.shares:.2f}, Cash: {self.cash:.2f}, Position: {self.position}")
...
if __name__ == '__main__':
    try:
        data_size = 2000
        dates = pd.to_datetime([pd.Timestamp('2023-01-01 00:00:00') + pd.Timedelta(minutes=15*i) for i in range(data_size)])
        price = 1.08 + np.cumsum(np.random.normal(0, 0.0005, data_size))
        spread_val = 0.0002
        dummy_data = {
            'open_bid': price - spread_val/2 - np.random.uniform(0, 0.0001, size=data_size),
            'high_bid': price - spread_val/2 + np.random.uniform(0, 0.0003, size=data_size),
            'low_bid': price - spread_val/2 - np.random.uniform(0, 0.0003, size=data_size),
            'close_bid': price - spread_val/2,
            'open_ask': price + spread_val/2 - np.random.uniform(0, 0.0001, size=data_size),
            'high_ask': price + spread_val/2 + np.random.uniform(0, 0.0003, size=data_size),
            'low_ask': price + spread_val/2 - np.random.uniform(0, 0.0003, size=data_size),
            'close_ask': price + spread_val/2,
            'volume': np.random.randint(100, 1000, data_size) * 100.0 # Ensure volume is float for stockstats if needed
        }
        forex_data = pd.DataFrame(dummy_data, index=dates)
        forex_data.index.name = 'timestamp'
        print("Dummy data created successfully. Shape:", forex_data.shape)
        # print("Data head:\n", forex_data.head())
