...
def fetch_data(source: str, instrument: str, timeframe: Timeframe, start_date: date, end_date: date) -> None:
    ...
    for d in days:
        ...
        if tick_data is None:
            logger.info(f"[{target_day}] No cached TICK data found. Downloading...")
            downloader = DOWNLOADERS[source]
            try:
                tick_data = downloader.fetch_day(instrument, target_day)
                tick_data.save_day(target_day)
            except Exception as e:
                logger.warning(f"[{target_day}] Could not fetch or process data for this day. Skipping. Reason: {e}")
                continue
        else:
            logger.info(f"[{target_day}] Found cached TICK data.")

        # 4. Downsample tick data and save the result
        logger.info(f"[{target_day}] Downsampling TICK data to {timeframe.name}...")
